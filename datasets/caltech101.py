...
def _build_transforms(
    ...
    """
    Constructs the training and evaluation transforms for Caltech101 dataset.
    ...
    if augment:
        train_transform = transforms.Compose(
            [
                ...
                transforms.ToTensor(),
                transforms.Normalize(CALTECH101_MEAN, CALTECH101_STD),
            ]
        )
    else:
        train_transform = transforms.Compose(
...
@DatasetRegistry.register("caltech101")
def load_caltech101(
    data_dir: str = "./data",
    batch_size: int = 64,
    shuffle: bool = True,
    ...
    """
    Loads the Caltech101 dataset with standardized transforms and DataLoaders.

    Caltech101 does not ship with official train/test splits, so a deterministic random
    ...
    Returns:
        ...
        A tuple containing:
            ...
            - The number of output classes (101 for Caltech101)
    """
    ...
    train_dataset_full = datasets.Caltech101(
        root=data_path,
        target_type="category",
        download=True,
        transform=train_transform,
    )
    eval_dataset_full = datasets.Caltech101(
        root=data_path,
        target_type="category",
        download=True,
