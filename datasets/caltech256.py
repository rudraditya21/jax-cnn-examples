...
CALTECH256_MEAN = (0.485, 0.456, 0.406)
CALTECH256_STD = (0.229, 0.224, 0.225)
...
def _build_transforms(
    ...
    """
    Constructs the training and evaluation transforms for Caltech256 dataset.
    ...
    if augment:
        train_transform = transforms.Compose(
            [
                ...
                transforms.ToTensor(),
                transforms.Normalize(CALTECH256_MEAN, CALTECH256_STD),
            ]
        )
    else:
        train_transform = transforms.Compose(
...
@DatasetRegistry.register("caltech256")
def load_caltech256(
    data_dir: str = "./data",
    batch_size: int = 64,
    shuffle: bool = True,
    ...
    """
    Loads the Caltech256 dataset with standardized transforms and DataLoaders.

    Caltech256 does not ship with official train/test splits, so a deterministic random
    split is used to form the training and evaluation subsets.
    ...
    Returns:
        ...
        A tuple containing:
            ...
            - The number of output classes (256 for Caltech256)
    """
    ...
    base_dataset = datasets.Caltech256(root=data_path, download=True)
    ...
    train_dataset_full = datasets.Caltech256(
        root=data_path,
        download=True,
        transform=train_transform,
    )
    eval_dataset_full = datasets.Caltech256(
        root=data_path,
        download=True,
